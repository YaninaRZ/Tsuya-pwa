...
export default function HabitCreate() {
    ...
    return (
        <div className="min-h-screen flex flex-col" style={{ background: '#0A0A0A' }}>
                        ...
                        <CardContent className="flex flex-col gap-4 p-4">
                            <div className="flex items-center justify-between">
                                ...
                                <Switch
                                    ...
                                    onCheckedChange={set('reminder_enabled')}
                                />
                            </div>
                            {form.reminder_enabled && (
                                <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.04] border border-[#C0C0C0]/20">
                                    ...
                                    <input
                                        ...
                                        className="outline-none bg-transparent text-sm text-white"
                                    />
                                </div>
                            )}
                        </CardContent>
                        ...
                        <Card>
                            <CardContent className="flex flex-col gap-3 p-4">
                                <div className="flex gap-2 flex-wrap">
                                    {DAYS.map((day) => {
                                        const active = form.reminder_days.includes(day.id)
                                        return (
                                            <Button
                                                key={day.id}
                ...
                <div className="flex flex-col gap-3 mb-8">
                    ...
                    <div className="flex rounded-2xl p-1 bg-white/[0.04] border border-white/[0.08]">
                        {['build', 'quit'].map((t) => (
                            <Button
                                key={t}
                                variant={form.habit_type === t ? 'default' : 'ghost'}
            ...
            {/* Sticky footer */}
            <div className="relative z-10 sticky bottom-0 px-6 py-4 max-w-lg mx-auto w-full" style={{ background: 'linear-gradient(to top, #0A0A0A 80%, transparent)' }}>
                <Button
                    onClick={handleSave}
                    disabled={saving || !form.label.trim()}
                    size="lg"
                    className="w-full tracking-[3px]"
                    variant={form.label.trim() ? 'default' : 'secondary'}
                >
                    {saving
